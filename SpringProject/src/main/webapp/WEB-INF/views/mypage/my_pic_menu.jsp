...
<script type="text/javascript">
	...
	function PicsOfThisMenu(folderName){
		...
		iframePic.src="picture/my_r_pic_page.action?folderName="+encodedFolderName;
		 ...
		 var url = "<%=cp%>/cy/picture/menuList.action";
	...
	function deleteFolder(folderName){
		...
		var url = "<%=cp%>/cy/picture/deleteOneFolder.action";
	...
	$(document).ready(function(){
		 $("#addMenu").click(function(){
			 ...
			 $.ajax({
				 type:"POST",
				 url:"<%=cp%>/cy/picture/createMenu_ok.action",
				 data:params,
		 ...
		 $("#deleteMenu").click(function(){
			 ...
			 $.ajax({
				 type:"POST",
				 url:"<%=cp%>/cy/picture/showDeleteMenuButton.action",
				 data:params,
				 success:function(args){
					 	$("#menuData").html(args);
		...
		$("#stopDeleteMenu").click(function(){
			 ...
			 $.ajax({
				 type:"POST",
				 url:"<%=cp%>/cy/picture/menuList.action",
				 data:params,
				 success:function(args){
					 	$("#menuData").html(args);
	...
	function checkCreateMenu(){
		...
		if(!str){
			swal("사진첩 이름을 입력하세요.", "", "<%=cp %>/resources/images/cy_logo.png");
			return true;
		}
		
		var privacy = document.getElementById("privacy").value;
		if(privacy==-1){
			swal("공개범위를 선택해 주세요.", "", "<%=cp %>/resources/images/cy_logo.png");
			return true;
		}
	...
	function checkDeleteMenu(){
			...
			if(totalFolderData==0){
				swal("삭제할 사진첩이 없습니다.", "", "<%=cp %>/resources/images/cy_logo.png");
				return true;
			}
	...
	function sendEditedFolderName(index, originalFolderNameToBeEdited){
		...
		var url = "<%=cp%>/cy/picture/editFolderName.action";
		
		if(!editedFolderName){
			swal("수정할 사진첩 이름을 입력하세요.", "", "<%=cp %>/resources/images/cy_logo.png");
			return
		}
		...
		<c:set var="index" value="${status.index }"/>
				...
				<div style="border: 0px solid black;float: right;width: 10%;">
						<a href="javascript:openEditFolderNameTextBar(${index })"> <!-- ----------------------------------------------------- -->
							<img alt="no found" src="<%=cp%>/resources/images/folder_edit.png" height="13px" width="13px">
						</a>
				</div><br/>
				...
				<c:if test="${dto.folderName==folderName }">
					...
					<div style="border: 0px solid black;float: left;width: 10%;">
						<a href="javascript:deleteFolder('${dto.folderName }')">
							<img alt="no found" src="<%=cp%>/resources/images/folder_deleted.png" height="15px" width="15px">
						</a>
					</div><br/>
				</c:if>
				<c:if test="${dto.folderName!=folderName }">
