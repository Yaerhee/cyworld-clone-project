...
<script type="text/javascript">
	...
	function search() {
		...
		window.open("user_main.action?userId="+userKeyword,userKeyword,
	...
	function sendKeyword() {
		...
		$.get(
			...
			function(data) {
				...
				var html = "";
				// 검색어가 있는지 없는지
				if(userKeyword == decodeURIComponent(name[0]) || userKeyword == id[0]) {
					boolCheck = true;
				} else {
					boolCheck = false;
				}
				...
				if(id.length >= 1) {		
					...
					for(var i = 0; i<id.length-1; i++){
						...
						id[i] =  id[i];
						name[i] = decodeURIComponent(name[i]);

						html += "<option value=\"" + id[i] +"\" label=\"" + name[i] + "님의 미니홈피\"></option>";
		    ...
		    for (var i = 0; i < opts.length; i++) {
		      if (opts[i].value === val) {
		        ...
		        if('${session.getUserId() }' == ""){
		       	...
		       	window.open("user_main.action?userId="+userId,userId,
				"width=1090,height=600,location=no,status=no,scrollbars=no");
		        break;
		      }
		    }
